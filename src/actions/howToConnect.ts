...
export async function handleHowToSetup(ctx: Context) {
  const text =
    ...
    `⚠️ **Если интернет не работает или кнопка "горит", но сайты не грузятся:**\n` +
    `Зайдите в настройки профиля в приложении и установите **MTU** на значение **1280**. Это решит проблему с блокировками на некоторых операторах.\n\n` +
